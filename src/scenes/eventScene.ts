...
const EVENT_THEME = ['По-джедайски', 'По-старославянски', 'По-пацански']
...
const KANDINSKY_PROMPT = {
    ...
    'По-старославянски': 'Иллюстрация, изображающая сердечки, солнце и цветы в стиле славянских картин Максима Кулешовва сердечки в центре занимают главную роль',
...
ВАЖНО: Твой ответ должен быть задумчивым и философским."`,
        ...
        'По-старославянски': `Представь что ты говоришь на древнерусском. Перепиши текст для ${ctx.session.event.name} "${ctx.session.event.prompt}" используя стилистику ответов в старорусском стиле. ПРИМЕРЫ ОТВЕТОВ В СТИЛЕ ДРЕВНЕРУССКОЙ РЕЧИ: \nЗемнаго не сведая и небесных пытаешиЕдин есть раб во всяком дому — сам господин.\n Вещати умеют мнози, а разумети не вси.\n Уне есть человеку добра смерть, негли злой живот.\n ВАЖНО: Твой ответ должен быть задумчивым и философским`,

        'По-пацански': `Представь что ты уличный хулиган. Перепиши текст для ${ctx.session.event.name} "${ctx.session.event.prompt}" используя стилистику ответов на уличном жаргоне. ПРИМЕРЫ ОТВЕТОВ В СТИЛЕ УЛИЧНОГО ХУЛИГАНА:\n
